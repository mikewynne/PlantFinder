...
var theArcGISServerName = "http://50.17.237.182/arcgis/rest/services/PlantSFv2/MapServer";
...
function identify(overlay, latLng) {
 ...
 function idresultsort(a, b) {
	...
	var d1 = a.attributes["Common_Name"];
	var d2 = b.attributes["Common_Name"];
	if (d1 < d2)
	return -1
	if (d1 > d2)
	return 1
...
//alert("in updatePlantListHtml")
	...
	require(["esri/tasks/query", "esri/tasks/QueryTask","dojo/dom", "dojo/on", "dojo/domReady!"], function(Query, QueryTask,dom, on){
		...
		query.outFields = ["Common_Name","Latin_Name","Former_Latin_Name","Plant_Type","Plant_Communities","Bloom_Time","Appropriate_Location","Flower_Color","Size_at_Maturity","Climate_Appropriate_Plants","Suitable_Site_Conditions","Soil_Type","Pruning_Needs","Water_Needs","Habitat_Value","Associated_Wildlife","Additional_Characteristices___Notes","Street_Tree_List","Suggested_for_Green_Connection_Routes","Stormwater_Benefit"];
		...
		function showResultsQuery(results) {
			...
			for (var i=0, il=results.features.length; i<il; i++) {
				...
				s+='<td align="middle" valign="top" ><img height="90px" width="90px" onClick="plantDetail(' + "'" + i + "'" +');" src="' + imgURL + '" alt="' + theLatinName+ '"></td><td valign="top" width="162"><a href="javascript:void(0);" onClick="plantDetail(' + "'" + i + "'" +');">'  +  theCommonName +'</a><br><h2>' + theLatinName +'</h2></td>';
			...
			//	}
			//	imgURL = "images/plants/medium/" + theLatinName + "01.jpg"
			//	s+='<td height="92px" width="92px" align="middle"><img height="90px" width="90px" onClick="plantDetail(' + "'" + i + "'" +');" src="' + imgURL + '" alt="' + theLatinName + '"></td><td width="8px" ></td><td> <a href="javascript:void(0);" onClick="plantDetail(' + "'" + i + "'" +');">'  +  theCommonName +'</a><br><h2>' + theLatinName +'</h2></td>';
				
			//	if(isOdd(i)) {
			//		s+="</tr><tr><td colspan=4 height='2px'></td></tr>"
			//	}
			//}
			//s+="</table><br>"
			//prompt(s,s)
			...
			//dom.byId('floatingDivForDetail').style.visibility='hidden';  
			//dom.byId('floatingDivForFilter').style.visibility='hidden';  
...
//alert(theQueryType)
	...
	require(["esri/tasks/query", "esri/tasks/QueryTask","dojo/dom", "dojo/on", "dojo/domReady!"], function(Query, QueryTask,dom, on){
		...
		if (!theQueryType) {
			...
			if (theSQL2!="" && theQueryType!="plantname") {
...
function loadOnePlant() {
	...
	require(["esri/tasks/query", "esri/tasks/QueryTask","dojo/dom", "dojo/on", "dojo/domReady!"], function(Query, QueryTask,dom, on){
		...
		function showCustomResultsQuery(results) {
            ...
            theLastSummary = "We found <strong> " + results.features.length + "</strong> plant for " + plantToLoad + ":"

			//theLastSummary = "<strong>" + results.features.length + "</strong> " + theSearchString + ":"
			document.getElementById('searchResultsSummary').innerHTML = theLastSummary
			...
			for (var i=0, il=results.features.length; i<il; i++) {
				...
				imgURL = "images/plants/medium/" + theLatinName + "01.jpg"
				s+='<td width="95px" align="middle" valign="top" ><img height="90px" width="90px" onClick="plantDetail(' + "'" + i + "'" +');" src="' + imgURL + '" alt="' + theLatinName+ '"></td><td valign="top" width="162"><a href="javascript:void(0);" onClick="plantDetail(' + "'" + i + "'" +');">'  +  theCommonName +'</a><br><h2>' + theLatinName +'</h2></td>';
...
function printResults() {
	...
	printHTML += "<div id='searchResults' >"
	printHTML +="\n<span id='searchResultsSummary'>" + theLastSummary+"</span><br>"
